...
## 8. MTR_110 ann 15：保留（2026-09-08 用户确认）

**决定：保留该实例，不作为标注质量异常剔除。** 并确立通则——标注宿主与「框重叠最多的结构」不一致的实例一律保留，在 M1 分析中作为独立一层报告。记入 `RESEARCH_PLAN.md` §13.5。

**仍未定的是成因，但它不影响该决定。** 已排除的解释：
...
剩下三种可能，影像上分不出：撕裂信号越出了被分割的半月板边界；框画得偏大或整体上移；原始标注确有偏差。三者都不改变保留的决定，因为无论成因为何，这个实例的宿主都只能靠标注者给出、靠重叠率必然答错。

**保留的理由**：它的宿主在 1.25 mm 外、框内只有股骨软骨，重叠率必然答「股骨软骨」而标注者答「内侧半月板」——这正是关系模型要答对的题。把重叠率答错的实例逐个剔除，等于构造一个偏袒 seg-then-lookup 对照臂的测试集，赌注实验就失去意义了。单个实例统计上无影响，但处理原则决定另外 7 个。
...
| # | 事项 | 状态 |
|---|---|---|
| 1 | MTR_110 ann 15 保留还是剔除 | ✅ 已定：保留（§13.5） |
| 2 | 在 §5.1 补明宿主判定的 `pad=4` 邻域规则 | ✅ 已补（§5.1 D5 第 ③ 条） |
| 3 | §9.1 报告中增加「标注宿主 ≠ 最大重叠结构」一层的单独统计 | 待办（§13.5 落点） |
| 4 | 积液 116 例与韧带 38 例目前对 U_B 呈背景 | 待办（计划文档 F5） |
| 5 | 三处 teacher forcing 换成预测量 | 正式跑分前必做 |
| 6 | arm A（nnU-Net seg-then-lookup）尚未建立 | M1 判据的前提 |
