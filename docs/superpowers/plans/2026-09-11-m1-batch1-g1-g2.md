...
- Upstream: `Backbone(embed_dim=64)` (channels 64/128/256/512), `d_model=256`, `K=6`, 6 anatomy-decoder layers, 8 heads, full-resolution mask head; lesion head = dense centre heatmaps on the F1 grid with `M=20` decoded slots (Q17 amendment). No RoPE, no masked attention, no M_valid.
- Nothing is tuned on held-out scans: fixed step count, last checkpoint; a lesion slot is a detection when its most probable class is not no-object, which for the dense head means a peak score of at least 0.5. nnU-Net predictions come from `checkpoint_final.pth`.
...
> **Superseded in part (Q17 amendment at the end):** the lesion head is the dense centre-heatmap head of `anatobind/model/dense_head.py`, not the DETR decoder shown in this task; the model, loss and test code that were committed are in the amendment.
...
> **Superseded in part (Q17 amendment at the end):** lesion slots are decoded from the centre heatmaps with `decode_centres`; the committed `predict.py` and its tests are in the amendment.
...
## Amendment 2026-09-11 (2): dense centre-heatmap lesion head (user decision Q17)

While writing the trainer it became clear that the lesion decoder's cross-attention memory carries no absolute position: MONAI's Swin uses relative position bias only, the patch embedding does not pad, and the decoder added no positional encoding. On lesion-centred crops this was invisible, because the box was always near the centre. A controlled synthetic test (`scratchpad/loc_long.py`: one bright 6×12×12 block at a uniformly random place in a 32×64×64 noise volume, batch 8, AdamW 3e-4, evaluated on 200 held-out samples) gave:

```
                         1000 steps     2000 steps     3000 steps     (mean IoU / share with IoU >= 0.1)
DETR, no coordinates     0.007 / 3%     0.006 / 2%     0.008 / 3%
DETR, coordinates        0.006 / 2%     0.009 / 3%     0.011 / 4%
dense centre heatmaps    0.886 / 100%   0.921 / 100%   0.938 / 100%
constant centred box     0.011 / 4%     (no image at all)
```

The user chose the dense head. The optional `coords` switch of the query stack (commit 2dbc273) stays off. What changed:

- `anatobind/model/dense_head.py` (new): `CentreHead`, `centre_targets`, `centre_loss`, `decode_centres`.
- `anatobind/model/upstream.py`: `u_head = CentreHead(...)` replaces `u_dec = UBDecoder(...)`; the forward returns `heat`, `offset`, `size`, `feat` instead of `logits`, `boxes`, `u_embed`, `aux`.
- `anatobind/model/upstream_losses.py`: the lesion terms are `centre_loss` (focal heatmap loss, L1 on offset and log size); parts are `mask, presence, heat, offset, size`.
- `anatobind/eval/predict.py`: `decode_centres` gives the M slots; the saved keys (`cls_prob`, `boxes_vox`, `u_embed`) are unchanged, so Tasks 13 and 14 are untouched.


```python
# anatobind/model/dense_head.py
...
**Timing after the change** (GPU 6, still shared with the same other user's process, four whole volumes per step): 13.70 M parameters instead of 19.98 M, 6.39 s per step = 1.60 s per volume, peak 45.7 GiB, so 7,500 steps (30,000 volumes) take about 13 h per fold instead of about 21 h. Task 10's 48-hour check is re-applied on the real pilot.

---

## Execution notes 2026-09-12

- **Checkpoint 1 reached** (Tasks 1–6 done, report in `docs/verification/2026-09-11/REPORT.md`, 25 commits pushed). Class-aware lookup ceiling on m1r: family level 0.958 (m1: 0.968); the corrected masks are on the h5 grid (identity frame, 155/155); the dicom-track masks were off by up to 3.8 mm, which also explains MTR_150's old frame-gate failure.
- **nnU-Net integrity check** rejected the first m1r export: the corrected NIfTI's zooms differ from the image's by up to 1e-4 mm (0.79988 vs 0.79998 on MTR_112). The seg header now copies the image's zooms exactly (commit 5e85686); the re-export and `plan_and_preprocess` then passed with 0 errors (17 min, 4 procs). 3d_fullres plan: patch [96, 160, 160], spacing [0.8, 0.625, 0.625], batch 2; fold 0 = 1488 training / 217 validation cases.
- **Pilot OOM on GPU 6**: the first launch died at step 9 (`CUDA out of memory`, 654 MiB free) because two other jobs had started on GPU 6 in the meantime (one of them the user's own GSTMI run, 19 GB). Relaunched on **GPU 7** with `--grad-ckpt` (peak ~31 GB instead of ~46 GB, about 7% slower) and `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`; nnU-Net fold 0 runs on **GPU 6**. This swaps the plan's assignment but stays within the two cards the user allowed. The crashed run's directory was removed before relaunching so `metrics.jsonl` starts at step 1.
- **Budget check (Task 10 Step 6 / Task 11 Step 7)**: pilot on the shared GPU 7 ran at 19.6 s/step (40.8 h per fold, the data workers idle at 2% CPU each, the main process launch-bound); nnU-Net fold 0 on the shared GPU 6 took 1214–1539 s per epoch (14–18 days per fold), stopped after 3 epochs. GPUs 0–2 were idle. **User decision Q18 = a**: borrow GPUs 0 and 1. The pilot was restarted from scratch on GPU 0 (`--grad-ckpt` kept as insurance against later co-tenants); nnU-Net runs on GPU 1 with `nnUNetTrainer_250epochs` (`TRAINER_DIR` in `anatobind/nnunet/prepare.py` follows). Task 11's `checkpoint_final.pth` rule is unchanged; the results folder is `nnUNetTrainer_250epochs__nnUNetPlans__3d_fullres`.
- **Checkpoint 2 (fold-0 pilot, 2026-09-12)**: training is clean (7500 steps in 4.5 h, every term falling, the convergence rule says extend), but the lesion head does not generalise: on the 31 held-out scans the peaks on true lesions score no higher than the peaks elsewhere (0.17 vs 0.15 median; recall with all 20 slots 0.57, best IoU median 0.14), while on its own training scans it localises nearly everything (recall 0.96, best IoU 0.94). The mask head is under-trained on both (Dice 0.28–0.63; nnU-Net 0.81–0.88). Full record: `docs/verification/2026-09-12/fold0_pilot.md`. **User decision Q19 = a**: the gate path takes the annotated box and class and predicts only the anatomy; detection becomes an auxiliary report. Implemented in `scripts/eval_g1_g2.py` (`GATE_KEY = "given_bucket"`), recorded in RESEARCH_PLAN §13.6. The extension to 11,250 steps and folds 1–4 wait for the mask-loss rebalance below.
- **Mask-loss balance A/B (2026-09-12, fold 0, 500 steps of four volumes each, same seed)**: A = current weights and clip 1.0, B = heat weight 0.1 and clip 5.0. Mask loss over steps 451–500: A 0.699, B 0.713 (start 1.23 for both); heat 3.95 vs 4.66. The hypothesis that the heat term starves the mask head is rejected. The remaining explanation is the pixel side of the mask head: `F1` is the raw patch embedding (no attention block runs before it in `Backbone.forward`), so the queries dot with features that carry no context. Next: a top-down pixel decoder (F4→F1) feeding the mask head, tested the same way before any retraining.
- **Pixel-decoder A/B (2026-09-12, fold 0, 500 steps of four volumes, same seed)**: pilot head on raw F1 vs the top-down pixel decoder. Mask loss over steps 451–500: 0.699 vs 0.831 (heat 3.95 vs 3.64). The decoder is not better at this horizon, so the batch-1 folds keep the pilot head (`FULL["pixel_dim"] = 0`); the decoder stays in the code as an option for batch 2. The mask head is not on the gate's path: G2 uses nnU-Net masks, and the batch-2 binders can take their geometry from the same nnU-Net masks, which also makes the B0/B1/B2/B3 comparison share one anatomy upstream.
- **Batch-1 upstream folds**: five folds on GPU 0 with the pilot configuration and the rule-extended 11,250 steps (the fold-0 rule said extend), for the "B0 on our masks" column and the batch-2 embeddings.
