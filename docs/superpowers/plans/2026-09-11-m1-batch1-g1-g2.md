...
- Whole-volume training and evaluation at batch size 1. Depth (model-frame Z) is zero-padded at the end to a multiple of 32; padded slices contribute to no loss and no metric.
...
CUDA_VISIBLE_DEVICES=6 PYTHONNOUSERSITE=1 PYTHONPATH=. setsid nohup nice -n 19 \
  ~/anaconda3/envs/nvgen/bin/python scripts/train_upstream.py --fold 0 --steps 30000 --out runs/upstream_fold0 --resume \
...
print(f"step {r['step']}: {per:.2f} s/step -> 30000 steps in {per * 30000 / 3600:.1f} h; loss {r['loss']:.3f}")
PY
```
...
The rule uses training loss only: if the mean total loss over steps 27,001–30,000 is more than 2% below the mean over steps 24,001–27,000, the schedule is too short and all five folds, fold 0 included, are trained with `--steps 45000` instead.
...
a = np.mean([loss[s] for s in range(24001, 27001)])
b = np.mean([loss[s] for s in range(27001, 30001)])
drop = (a - b) / a
print(f"mean loss 24k-27k {a:.4f}, 27k-30k {b:.4f}, relative drop {drop:.3%} -> "
      f"{'EXTEND to 45000 steps' if drop > 0.02 else 'converged: keep 30000 steps'}")
PY
```
...
CUDA_VISIBLE_DEVICES=6 setsid nohup bash -c 'for f in 1 2 3 4; do \
  ...
  --fold $f --steps 30000 --out runs/upstream_fold$f --resume > logs/upstream_fold$f.log 2>&1; done' \
  > /dev/null 2>&1 &
```
