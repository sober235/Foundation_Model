...
- 测试:`PYTHONNOUSERSITE=1 PYTHONPATH=. ~/anaconda3/envs/nvgen/bin/python -m pytest tests/ -q -p no:cacheprovider`(2026-09-23 计划 1 合入后:342 passed,约 50 s)。仓库没有 CI,评审 PR 时把分支导出到临时目录跑:`git archive <ref> | tar -x -C <dir>`。
- **当前状态:计划 1"膝侧能力系统"已交付并合入 main(tag `handoff/2026-09-23-knee-capability`):三条能力的代码、单命令入口 `scripts/infer_knee.py`、Dataset902 五折病灶检测器与五折数字都在;**检测门不过**(大类正确灵敏度 0.254 @ 1.57 FP/卷,门 ≥ 0.5),按计划停下等用户定第二臂(`docs/verification/2026-09-23/knee_eval/VERDICT.md` §4)。先读 `STATUS.md`,再读 `docs/superpowers/specs/2026-09-23-aur-capability-system-design.md`(决定记录)、`docs/superpowers/plans/2026-09-23-knee-capability-system.md`、`docs/verification/2026-09-23/knee_eval/{REPORT,VERDICT}.md`。v2.6 的 Gate 链仍有效,Gate 0 并入计划 3;别再在 SKM-TEA 膝关节上找"退化让绑定失效"(G2 裁决已测死),也别把 SynthSeg 查表当脑侧真值。**每个函数先写测试;结构性测试是"loss 在降但标签错了"的唯一自动防线。
...
- `anatobind/eval/`:`predict.py`(缓存预测)、`lookup.py`(类别感知 B0 + 零重叠最近兜底,PR-C 在此扩展)、`matching.py`(IoU 匹配、分桶)、`g2.py`(配对差、扫描 bootstrap)、`detect3d.py` + `gate.py`(leg 2 的 3D 聚合与 H1/H2/H3 门)。`anatobind/nnunet/prepare.py`(nnU-Net 数据集与划分)、`anatobind/nnunet/lesion_labels.py`(箱填病灶标签)、`anatobind/nnunet/prepare_lesion.py`(Dataset902)、`anatobind/eval/lesion_boxes.py`(连通域取框、npz 概率读取)、`anatobind/eval/detection_metrics.py`(灵敏度/假阳/工作点/门)、`anatobind/infer/{canonical,knee}.py`(入口:帧规范化、nnU-Net 子进程、病灶表、叠图)。`anatobind/data_engine/rawtrack_fetch.py`、`seg_frames.py`(校正版分割获取与坐标系)。
- `scripts/`:`build_skmtea_m1.py`(历史)、`build_skmtea_m1r.py`、`build_m1r_cache.py`、`fetch_skmtea_rawtrack_api.py`、`discover_rawtrack_frame.py`、`train_upstream.py`、`cache_predictions.py`、`nnunet_prepare.py` + `nnunet_env.sh`、`nnunet_prepare_lesion.py`、`collect_nnunet_predictions.py`、`eval_g1_g2.py`、`eval_knee_folds.py`(五折检测/绑定/解剖三张表)、`infer_knee.py`(单命令入口)、`peek_fold.py`;旧的 `train_armb_minimal.py`、`render_binding_overlay.py`、`run_synthseg_fastmri_brain.py`、`verify_skmtea_frames.py`。
- 工作树:`../foundation_model-aur`(分支 build/aur-system,计划 1 在此完成并已合入 main;`../foundation_model-armb` 已不存在)。
