...
2. **Gate A/U**：验证 anatomy parsing 与 lesion type + 3D localization；
3. **Gate R0**：约 100–150 lesion 双阅片 pilot，冻结 relation ontology、reader agreement、ambiguity 与抽样/加权规则，再估最终 N；
...
**当前 Gate 系统**:Gate 0 数据坐标正确性 → Gate A/U 基础感知 → Gate R0 独立人标 relation truth 与可标注性 → Gate R1 learned relation 的增量价值 → Robustness / E。clean SynthSeg + geometry pseudo-reference 只用于训练、debug 与分层，不定义 superiority。
...
> **v2.5 历史章节，非当前默认实现。** 当前 relation 主线以 v2.5 的 B0–B5 为准：B1=IndependentCandidateHead，B3=同一 lesion 内 candidate interaction，B4=B3+local boundary evidence；旧 global K×M RelationModule 仅作 ablation。
...
> **v2.5 历史章节，Gate R1 通过前不执行。** E 已降为 residual relation failure 的后续 selective-reliability 扩展。
...
> **v2.5 历史训练纲领，非当前执行顺序。** 当前执行顺序为 Gate 0 → A/U → Level R pilot/final cohort → relation baselines → learned relation → robustness → E。
...
> **v2.5 历史实验框架，待后续按 v2.5 Gate 重新实现。** 第一篇当前 primary question 是 independent Level R 上 B4 是否 beyond Bgeo+ / B2。
...
> **v2.5 历史扩展设计。** 多器官/组合泛化不进入第一篇最小闭环，第一篇以 brain focal lesion 为 R 主战场、SKM-TEA 为 geometry-easy control。
...
> **v2.5 历史扩展设计。** 仅在 Gate R1 通过并完成 robust-perception control 后重新启用。
...
> **v2.5 历史指标表。** 当前 R 主统计以 all-lesion host accuracy/macro-F1/coverage/rescue/harm/net rescue + patient-level bootstrap 为准；富集抽样必须做 stratified standardization 或 IPW。
...
> **v2.5 历史风险表。** 仍可用于审计，但当前最高优先级风险已更新为：坐标正确性、U 可用性、Level R 可标注性/reader availability、sampling bias、Bgeo+ ceiling。
...
- **M1**(2026-09-13 结果):第一批完成。G1 过;**G2 不过**——判门路径(标注框和类别 + nnU-Net 解剖)下,组织族错误率 clean 0.039、noise_q3 与 us16 均 0.035,配对差 −0.003(95% CI 不含正值);V5 §15.2 的 geometry-conflict 子集为空(311 个病灶 × 6 档退化 = 0 例)。按 §13.6 原定"G2 不过 → 先补运动仿真",但补测三项后不执行该分支,理由见 §13.7。
...
**v2.5 解释**：该裁决证明 SKM-TEA 不是 relation robustness 的合适主战场，但不再作为整个 A/U/R structured perception 的停止条件。

**原裁决**：G2 不过。退化不让公平查表把病灶绑到错的组织族。

**为何不按原定去补运动仿真**，三项补测：

1. 退化几乎没伤到解剖。16 倍欠采只掉 2 个点 Dice、结构质心移 0.46 mm，比我们刚换掉的旧 dicom-track 分割自身的偏差（中位 0.3–0.9 mm）还小。
2. 任务对解剖质量本来就不敏感。同样扫描只换解剖来源：真值 3.4%、nnU-Net 2.2%、本模型（Dice 0.40）6.7%。Dice 砍一半只动 4.5 个点。
3. 门要多大位移才响：5–8 mm。只训干净图的分割模型（分布外）在 16 倍欠采下给 1.02 mm，仍差五倍。运动要做到 5–8 mm，临床会重扫。

V5 方案（`docs/AnatoBind_MRI_完整技术方案_V5_排版校正版.pdf`）在第 1 页、§15.1、图 4、§17 四处写了同一条停止规则，其 §17 明确把运动排在"前四步都成立以后"。第三步失败，故不执行运动分支。

**余量由什么决定**（`size_vs_margin.py`）：膝侧类别限定压倒一切，病灶大小几乎不影响；脑侧无类别限定时尺度决定一切，边长 3 mm 的病灶 1 mm 位移就有 11.7% 改判宿主，是膝侧的十倍。存在能激活该科学问题的配置——小病灶 + 无类别限定 + 结构密集器官；障碍是脑侧缺少标注者指定的病灶–结构对应真值，主张会从"更准"退成"更一致"。
...
**评估口径**:检出 = 该事件槽位最可能的类别不是 no-object(密集头下即峰值得分 ≥ 0.5);与真值按 3D IoU 一对一匈牙利匹配,阈值 0.1(另报 0.05、0.25),匹配不看类别。每个分割内病灶归入:漏检 / 检出但类别错 / 类别对但组织族错 / 正确。组织族真值取 tissue_id,全部 311 个分割内实例都可评分;标签级(含侧别)只在宿主已解析的实例上报告。

**2026-09-12 修订(fold-0 试跑之后,用户确认 Q19)**:从零训的检测头在留出扫描上不泛化(训练扫描全槽位召回 0.96、留出 0.57,留出扫描上命中与落空槽位的分数分不开,按 ≥0.5 规则检出 0/84),G2 要比的"两图都检出"集合为空。改为:**每个标注病灶以标注的框和类别进入查表,只预测解剖**(nnU-Net 分割定 G2,本模型分割只报告);G1 改为解剖与关系路径上不喂真值;检测降为附带报告(留出召回、分桶、按预测框算的 G2 表)。这是看过 fold 0 结果后改的门定义,论文里明写。G2 的 +5 个点、配对集合(两图上都有该病灶,即全部分割内病灶)、bootstrap 规则不变;分桶只剩 正确 / 组织族错。第二批的关系模型同样以标注框为输入。
...
**模型与训练**:
- 整卷训练、整卷评估;训练每步 4 个整卷(2026-09-11 用户确认)。GPU 6、7 与他人共用,实测每次下发算子排队约 1.2 ms,一步约 4800 次下发、GPU 实算仅 0.31 s:每步 1 个整卷时 5.7 s/整卷、单折约 48 h;每步 4 个时 2.5 s/整卷、单折约 21 h,峰值 49 GB。深度补零到 32 的整数倍,同一批按最深者补齐,补出的层不计损失、不计指标。步数按整卷计:7500 步 × 4 = 3 万个整卷,预热 125 步。换成密集中心热图头后重测(同一张卡、同样共用):1.6 s/整卷,单折约 13 h,峰值 46 GB。
- 上游 = MONAI Swin(embed 64,通道 64/128/256/512)+ 身份锚定 A(全分辨率 mask 头:F1 网格面内 2.5 mm,而软骨只有 2–3 mm 厚)+ 密集中心热图 U_B(F1 网格上逐类中心热图 + 格内偏移 + 框大小,解码取 20 个峰作事件槽位,事件 embedding 取峰值处特征);d_model 256。不加 RoPE、masked attention、M_valid,留作消融。
- U_B 由 DETR 查询换成密集中心热图头(2026-09-11 用户确认):合成对照实验(随机位置放一个亮块),无参考点的 DETR 解码器在 2.4 万个样本后平均 IoU 仍为 0.01(给查询记忆加坐标编码亦然),中心热图头 8000 个样本即达 0.89、2.4 万达 0.94。
