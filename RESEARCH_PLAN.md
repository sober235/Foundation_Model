...
**第一道门**(M1,6–8 周,§9.1):SKM-TEA 155 例按 scan 五折交叉验证,分割内 311 个可用病理实例(纳入规则见 §5.1),损坏条件下,显式关系模型 vs seg-then-lookup,ABA 提升 ≥ 10 个百分点且配对检验显著;不过则方向重议,不进 M2。
...
- **判据口径**:全 155 例按 scan 五折 CV(155 scan = 155 受试者,无泄漏),分割内 311 例实例,三级退化条件,ABA 提升 ≥ 10 个百分点且 instance 级 paired bootstrap / McNemar 显著;官方 split 86/33/36 只作报告口径。积液 116 例作弃权分析,韧带 38 例作扩展分析,三者分开报。
...
可用实例:官方 test 分割内 84、全部 130(筛前);五折 CV 分割内 311、全部 465(纳入规则后)。折扣:实例按扫描聚簇(平均每例约 3 个),设计效应 1.3–1.5,有效样本约为名义值 70%;CV 汇总各折预测再做配对检验略偏乐观。折算后官方 test 只能检出 ≥ 15 个百分点;全 155 例 CV 分割内 311 例(有效约 215–240)可检出 ≥ 9–10 个百分点。**门槛定 10 个百分点。**
