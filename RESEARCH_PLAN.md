...
- **SKM-TEA 标注纳入规则(2026-09-06 确认)**:① 框的 w/h/d 任一非正、或端点排序后仍越界的记录剔除,共 9 条(主组 7、积液 1、韧带 1;其中 train/191 MTR_161 的 z 端点 271 > 160,val/29 MTR_047 的 x 端点 2478 > 512);② 类别与 tissue_id 不相容的记录剔除,共 2 条(train/48 MTR_145 韧带撕裂 → 半月板;test/117 MTR_156 软骨病变 → ACL);原始标注文件不改,剔除清单与理由随数据引擎 manifest 记录。③ 宿主侧别:tissue_id 只给组织族(半月板、胫骨软骨不分内/外侧),侧别由框与该组织内/外侧分割 mask(标签 3/4、5/6)的重叠比决定,重叠比落在 0.4–0.6 的记为侧别模糊并单独报告。④ 待数据引擎落实:标注 JSON 的方位元数据(137 例 SI/AP/LR、18 例 SI/AP/RL)与分割 NIfTI 轴码 P/I/R 的映射须在真实数据上核对后固化;六个 Poisson 掩膜为 416×(depth−80),嵌入 ky–kz 网格的规则须明写;4 卷深度非 160(168/152/156/144)。
