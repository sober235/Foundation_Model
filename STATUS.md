...
- 已定（用户 2026-09-25 拍板，v2.6 §25 第 9 项）：**Gate 0.5 判 GO，不分困难/简单两组，t 不冻结**；分析分层改为 d_interface 四档 0 / 0–2 / 2–4 / >4 mm（42 / 16 / 17 / 25%），Level R 标注按全集 1297 做，不做富集抽样、不需要 IPW；3 mm 层厚卷上的 43 个病灶单列。接下来写 Level R 协议与工具（PR-B）。
...
2. Level R 协议与工具（v2.6 PR-B）：标注字段、导出、盲化工具、一致率统计、封存目录；pilot 改为按四档 × 采集分层覆盖；读片人仍待填名。
