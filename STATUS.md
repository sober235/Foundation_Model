...
- **09-13 之后的证据链**(都在仓库里):
  ...
  - 评审文件:`REVIEW_v7_feasibility_2026-09-16.md`(SynthSeg+ 就是已用的 --robust;DETR 应换热图头)、`REVIEW_v7_feasibility_2026-09-19.md`(Gate A′ / Gate B 两道生死门)、`REVIEW_core_target_AUR_2026-09-22.md`(A/U/R 核心目标)、`REVIEW_v2.5_feasibility_2026-09-22.md`(v2.5 合入后的冷启动独立评审:**有条件 GO**;余量薄、脑侧聚簇 m_eff 29.45 使功效被高估、单一留出检不出、Level R 读片 65–173 小时;§7 对 §25 五项的建议、§9.1 五个条件与停止规则、§9.3 两周动作;计数脚本与原始输出在 `docs/verification/2026-09-22-feasibility-review/`)。
...
- 关键实测数字(判断余量用):SKM-TEA 类别感知查表 clean 天花板 0.958(m1r)–0.968(oracle),余量约 3 点,311 例检不出(§9.7:d=0.03 需约 700 例);fastMRI+ 脑有框 476 卷,FLAIR 252 患者、1825 病灶、1297 小病灶(165 卷);探针 24 卷里约 79% 小病灶的查表宿主是白质(614/780,全集未测)。
...
- **关系模型评估用患者级五折 CV 还是单一留出**:CV 让全部 1297 小病灶当测试,人标目标 = 全集(两读者合计约 2600 例、各约 1300 例,每例 1.5 分钟约 32 小时/人,裁定约 260 例;脑侧实测聚簇 m_eff 29.45,ICC 0.05 时设计效应 2.42,见新评审 §2.2);单一 20% 留出只有约 260 病灶,全标也只能检出 6–8 个点。退路 = ≥700 例加权子集(p_disc 0.10、d 0.04 需 640–735)。
...
新评审(09-22)建议与 PR-A 并行插入一步:把探针的脑侧读框/合并/查表移入仓库并加测试,对全部 1297 个小病灶在干净 SynthSeg 上算宿主分布、到最近其他候选边界的距离(t = 2/3/5 mm 下的 H1 占比)、单层比例、每患者病灶数(CPU 几分钟),先算清分歧预算再投入读片(评审 §9.3 动作 3);并把 §25 五项按评审 §7 一次拍完。

估时:Gate 0 一两天;H1 重跑两三天 GPU;pilot 取决于医生;基线两三周。评审估 PR-A 到 PR-D 合计 8–12 周工程,Gate R1 最早第五到六个月(受读片速度约束)。仍无正式时间表。
